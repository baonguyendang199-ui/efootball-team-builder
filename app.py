...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Nhập thông tin các cầu thủ bạn đang cân nhắc để hệ thống phân tích mức độ ưu tiên.")
        ...
        with st.form("scout_form"):
            ...
            for i in range(4):
                with cols[i]:
                    ...
                    name = st.text_input(f"Tên", key=f"sc_name_{i}")
                    ...
                    club = st.text_input(f"CLB", key=f"sc_club_{i}")
                    nation = st.text_input(f"Quốc gia", key=f"sc_nation_{i}")
                    p_type = st.selectbox(f"Loại thẻ", ["NON-EPIC", "EPIC", "POTW"], key=f"sc_type_{i}")
                    if name:
                        candidates.append({"Player": name, "Rating": rat, "Position": pos, "Club": club, "Nation": nation, "Player Type": p_type, "Epic_Priority": 0 if p_type == "EPIC" else 1})
