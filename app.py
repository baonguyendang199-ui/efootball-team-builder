...
def show_player_modal(row):
    """Hiển thị thông tin chi tiết cầu thủ trong Popup (UI Pro Version + WF 0-3)"""
    ...
    if "EPIC" in p_type:
        ...
        bg_gradient = "linear-gradient(135deg, #451a03 0%, #b45309 100%)"
        border_color = "#fbbf24"
    elif "POTW" in p_type or "TRENDING" in p_type:
        theme_color = "#d946ef" # Purple
        bg_gradient = "linear-gradient(135deg, #4c1d95 0%, #7e22ce 100%)"
        border_color = "#e879f9"
    else:
    ...
    # --- 2. HÀM CHUYỂN ĐỔI WEAK FOOT SANG 0-3 ---
    def get_wf_data(val, mode='usage'):
        text = str(val).upper().strip()
        score = 1 # Default
        
        if mode == 'usage': # Tần suất
            if "REGULARLY" in text: score = 3
            elif "OCCASIONALLY" in text: score = 2
            elif "RARELY" in text: score = 1
            elif "ALMOST" in text or "NEVER" in text: score = 0
        else: # Độ chính xác
            if "VERY HIGH" in text: score = 3
            elif "HIGH" in text: score = 2
            elif "MEDIUM" in text: score = 1
            elif "LOW" in text: score = 0
            
        # Màu sắc dựa trên điểm số
        colors = {
            3: "#a855f7", # Tím (Max ping)
            2: "#3b82f6", # Xanh (Tốt)
            1: "#f97316", # Cam (TB)
            0: "#64748b"  # Xám (Kém)
        }
        return score, colors.get(score, "#64748b")

    wf_use_score, wf_use_color = get_wf_data(row.get('Weak Foot Usage', ''), 'usage')
    wf_acc_score, wf_acc_color = get_wf_data(row.get('Weak Foot Accuracy', ''), 'acc')
    ...
    st.markdown(f"""
        ...
        .e-info {{ flex-grow: 1; z-index: 2; color: white; }}
        .e-name {{ font-size: 1.8rem; font-weight: 800; line-height: 1.1; margin-bottom: 4px; text-transform: uppercase; letter-spacing: -0.5px; }}
        .e-meta {{ font-size: 0.9rem; opacity: 0.9; display: flex; gap: 10px; align-items: center; }}
        ...
        .e-rating-val {{ font-size: 2.2rem; font-weight: 800; line-height: 1; color: {theme_color}; }}
        .e-rating-lbl {{ font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px; opacity: 0.8; }}
        ...
        .stat-card:hover {{ background: rgba(30, 41, 59, 0.8); border-color: rgba(255,255,255,0.1); }}
        
        .stat-lbl {{ font-size: 0.7rem; text-transform: uppercase; color: #94a3b8; margin-bottom: 4px; letter-spacing: 0.5px; text-align: center; }}
        ...
        .wf-score {{ font-size: 1.2rem; font-weight: 800; }}
        .wf-sub {{ font-size: 0.6rem; opacity: 0.7; }}
        ...
        .skill-base {{ background: rgba(59, 130, 246, 0.1); color: #60a5fa; border: 1px solid rgba(59, 130, 246, 0.2); }}
        .skill-add {{ background: rgba(34, 197, 94, 0.1); color: #4ade80; border: 1px solid rgba(34, 197, 94, 0.2); }}
    ...
    st.markdown(f"""
        <div class="e-header">
            <img src="{real_img}" class="e-avatar">
            <div class="e-info">
                <div class="e-name">{p_name}</div>
                <div class="e-meta">
                    <span style="background:rgba(255,255,255,0.15); padding:2px 8px; border-radius:4px; font-weight:600;">{pos}</span>
                    <span>{row.get('Club','')}</span>
                    <span style="opacity:0.5">•</span>
                    <img src="https://flagcdn.com/24x18/{get_nation_code(row.get('Nation',''))}.png" style="height:14px; opacity:0.8;" onerror="this.style.display='none'"> 
                    <span>{row.get('Nation','')}</span>
                </div>
            </div>
            <div class="e-rating-box">
                <div class="e-rating-val">{rating}</div>
                <div class="e-rating-lbl">OVR</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div class="stat-grid">
            <div class="stat-card">
                <div class="stat-lbl">Thể hình</div>
                <div class="stat-val">{row.get('Height','-')} <span style="font-size:0.8em;opacity:0.7">cm</span></div>
                <div class="stat-val" style="font-size:0.9em; opacity:0.8">{row.get('Weight','-')} <span style="font-size:0.8em;opacity:0.7">kg</span></div>
            </div>
            <div class="stat-card">
                <div class="stat-lbl">Tuổi / Chân</div>
                <div class="stat-val">{row.get('Age','-')} <span style="font-size:0.8em;opacity:0.7">tuổi</span></div>
                <div class="stat-val" style="font-size:0.9em; opacity:0.8">{row.get('Foot','-')}</div>
            </div>
            <div class="stat-card">
                <div class="stat-lbl">Phong độ</div>
                <div class="stat-val" style="color: #60a5fa">{row.get('Form','-')}</div>
                <div class="stat-lbl" style="margin-top:4px">Chấn thương</div>
            ...
            <div class="stat-card" style="justify-content:space-between; padding:8px;">
                ...
                <div class="wf-container">
                    <div class="wf-badge">
                        <div class="wf-score" style="color:{wf_use_color}">{wf_use_score}</div>
                        <div class="wf-sub">Usage</div>
                    </div>
                    <div class="wf-badge">
                        <div class="wf-score" style="color:{wf_acc_color}">{wf_acc_score}</div>
                        <div class="wf-sub">Acc</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    if not skills and not added_skills:
        ...
        skill_html = '<div>'
        for s in skills:
            skill_html += f'<span class="skill-tag skill-base">{s}</span>'
        for s in added_skills:
            skill_html += f'<span class="skill-tag skill-add">+{s}</span>'
        skill_html += '</div>'
    ...
    with c2:
        st.caption(f"ID: {row.get('Player ID', 'N/A')} • Type: {p_type} • League: {row.get('League', 'N/A')}")

# Helper function nhỏ để lấy mã cờ (Optional, nếu không cần có thể bỏ qua dòng img flag)
def get_nation_code(nation):
    # Mapping đơn giản để demo, thực tế cần list đầy đủ hoặc library pycountry
    common = {
        "Argentina": "ar", "Portugal": "pt", "France": "fr", "Brazil": "br", 
        "England": "gb-eng", "Germany": "de", "Spain": "es", "Italy": "it",
        "Belgium": "be", "Netherlands": "nl", "Vietnam": "vn", "Japan": "jp"
    }
    return common.get(str(nation).strip(), "un")
...
# --- FORMATION DATABASE (THE ULTIMATE COLLECTION V2) ---
# Constraints Checked: GK=1 | CB 2-3 | DMF 1-3 (soft limit) | CF 0-2 | SS 0-5
# Note: Một số sơ đồ "Flat" hoặc "No DMF" có thể có 0 DMF (chấp nhận theo logic bóng đá thực tế).

FORMATIONS = {
    # =================================================================
    # 🔥 ĐẶC BIỆT: NHÓM 2 CF + 1 SS (TAM TẤU TRUNG LỘ)
    # =================================================================
    "4-3-3 (2 CF, 1 SS)":           ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "CMF", "SS", "CF", "CF"],
    "4-2-1-3 (2 CF, 1 SS)":         ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "SS", "CF", "CF"],
    "4-1-2-3 (2 CF, 1 SS)":         ["GK", "LB", "CB", "CB", "RB", "DMF", "AMF", "AMF", "SS", "CF", "CF"],
    "3-4-3 (2 CF, 1 SS)":           ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "CMF", "SS", "CF", "CF"],
    "3-5-2 (2 CF, 1 SS)":           ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "AMF", "SS", "CF", "CF"],
    "5-2-3 (2 CF, 1 SS)":           ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "DMF", "SS", "CF", "CF"],
    
    # Sửa lỗi trùng key ở bản gốc
    "4-2-4 (1 CF, 1 SS, CMF duo)":  ["GK", "LB", "CB", "CB", "RB", "CMF", "CMF", "LWF", "RWF", "SS", "CF"], 
    "4-2-4 (1 CF, 1 SS, Mix Mid)":  ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "LWF", "RWF", "SS", "CF"], 

    # =================================================================
    # 🛡️ 4 HẬU VỆ (4-3-3 & BIẾN THỂ)
    # =================================================================
    "4-3-3 (Classic)":              ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "CMF", "LWF", "RWF", "CF"],
    "4-3-3 (Holding)":              ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "DMF", "LWF", "RWF", "CF"], # Busquets role
    "4-3-3 (Attack)":               ["GK", "LB", "CB", "CB", "RB", "DMF", "AMF", "AMF", "LWF", "RWF", "CF"], # Man City style
    "4-3-3 (False 9)":              ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "CMF", "LWF", "RWF", "SS"], # CF -> SS
    "4-3-3 (1 DMF, 2 SS)":          ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "CMF", "SS", "SS", "CF"],
    "4-3-3 (3 SS)":                 ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "CMF", "SS", "SS", "SS"],
    "4-3-3 (Flat CMF)":             ["GK", "LB", "CB", "CB", "RB", "CMF", "CMF", "CMF", "LWF", "RWF", "CF"],
    "4-3-3 (Asymmetric Left)":      ["GK", "LB", "CB", "CB", "RB", "DMF", "LMF", "CMF", "AMF", "SS", "CF"],  # Lệch trái

    # =================================================================
    # 🛡️ 4 HẬU VỆ (4-2-3-1 & 4-2-1-3)
    # =================================================================
    "4-2-1-3 (2 DMF)":              ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "LWF", "RWF", "CF"],
    "4-2-1-3 (1 DMF, 1 CMF)":       ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "AMF", "LWF", "RWF", "CF"],
    "4-2-1-3 (3 SS)":               ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "SS", "SS", "SS"],
    "4-2-1-3 (No DMF)":             ["GK", "LB", "CB", "CB", "RB", "CMF", "CMF", "AMF", "LWF", "RWF", "CF"],

    "4-2-3-1 (Wide)":               ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "LMF", "RMF", "CF"],
    "4-2-3-1 (3 AMF)":              ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "AMF", "AMF", "CF"],
    "4-2-3-1 (Narrow)":             ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "LWF", "RWF", "CF"], # Giống 4-2-1-3
    "4-2-3-1 (1 DMF, 1 CMF)":       ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "AMF", "LMF", "RMF", "CF"],
    "4-2-3-1 (SS Shadow)":          ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "SS", "LMF", "RMF", "CF"],  # AMF -> SS

    # =================================================================
    # 🛡️ 4 HẬU VỆ (4-1-2-3 & 4-1-4-1)
    # =================================================================
    "4-1-2-3 (Standard)":           ["GK", "LB", "CB", "CB", "RB", "DMF", "AMF", "AMF", "LWF", "RWF", "CF"],
    "4-1-2-3 (Wide SS)":            ["GK", "LB", "CB", "CB", "RB", "DMF", "AMF", "AMF", "SS", "SS", "CF"],
    "4-1-2-3 (No DMF)":             ["GK", "LB", "CB", "CB", "RB", "CMF", "AMF", "AMF", "LWF", "RWF", "CF"],
    
    "4-1-4-1 (Attack)":             ["GK", "LB", "CB", "CB", "RB", "DMF", "LMF", "RMF", "AMF", "AMF", "CF"],
    "4-1-4-1 (Control)":            ["GK", "LB", "CB", "CB", "RB", "DMF", "LMF", "RMF", "CMF", "CMF", "CF"],
    "4-1-4-1 (False 9)":            ["GK", "LB", "CB", "CB", "RB", "DMF", "LMF", "RMF", "CMF", "CMF", "SS"],

    # =================================================================
    # 🛡️ 4 HẬU VỆ (4-4-2 CLASSIC & BIẾN THỂ)
    # =================================================================
    ...
    "4-4-2 (Diamond Wide)":         ["GK", "LB", "CB", "CB", "RB", "DMF", "LMF", "RMF", "AMF", "CF", "CF"],
    "4-4-2 (Box Magic)":            ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "AMF", "CF", "CF"],
    "4-4-2 (2 SS)":                 ["GK", "LB", "CB", "CB", "RB", "LMF", "RMF", "DMF", "CMF", "SS", "SS"],
    "4-4-1-1 (Support)":            ["GK", "LB", "CB", "CB", "RB", "LMF", "RMF", "CMF", "CMF", "SS", "CF"], # 1 CF lùi thành SS

    # =================================================================
    # 🛡️ 4 HẬU VỆ (4-2-2-2 & 4-2-4)
    # =================================================================
    "4-2-2-2 (2 AMF)":              ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "AMF", "CF", "CF"],
    "4-2-2-2 (LWF/RWF)":            ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "LWF", "RWF", "CF", "CF"],
    "4-2-2-2 (1 CF, 1 SS)":         ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "AMF", "SS", "CF"],
    
    "4-2-4 (Standard)":             ["GK", "LB", "CB", "CB", "RB", "CMF", "CMF", "LWF", "RWF", "CF", "CF"],
    "4-2-4 (2 DMF)":                ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "LWF", "RWF", "CF", "CF"],
    "4-2-4 (4 SS)":                 ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "SS", "SS", "SS", "SS"],

    # =================================================================
    # 🛡️ 4 HẬU VỆ (CÂY THÔNG & KHÁC)
    # =================================================================
    ...
    "4-5-1 (Defensive)":            ["GK", "LB", "CB", "CB", "RB", "LMF", "RMF", "DMF", "DMF", "CMF", "CF"],
    "4-5-1 (Flat)":                 ["GK", "LB", "CB", "CB", "RB", "LMF", "RMF", "CMF", "CMF", "CMF", "CF"],

    # =================================================================
    # 🛡️ 3 HẬU VỆ (3 CB - TẤN CÔNG)
    # =================================================================
    "3-4-3 (Flat)":                 ["GK", "CB", "CB", "CB", "LMF", "RMF", "CMF", "CMF", "LWF", "RWF", "CF"],
    "3-4-3 (Diamond Mid)":          ["GK", "CB", "CB", "CB", "DMF", "CMF", "LMF", "RMF", "SS", "SS", "CF"],
    "3-4-3 (2 AMF)":                ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "DMF", "AMF", "AMF", "CF"],
    "3-4-3 (3 SS)":                 ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "CMF", "SS", "SS", "SS"],
    "3-4-3 (Cruyff Diamond)":       ["GK", "CB", "CB", "CB", "DMF", "LMF", "RMF", "AMF", "SS", "SS", "CF"], # Biến thể 3-4-3 kim cương

    "3-5-2 (Standard)":             ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "CMF", "AMF", "CF", "CF"],
    "3-5-2 (3 CMF)":                ["GK", "CB", "CB", "CB", "LMF", "RMF", "CMF", "CMF", "CMF", "CF", "CF"],
    "3-5-2 (2 DMF)":                ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "DMF", "AMF", "CF", "CF"],
    "3-5-2 (3 DMF)":                ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "DMF", "DMF", "CF", "CF"],
    "3-5-2 (1 SS, 1 CF)":           ["GK", "CB", "CB", "CB", "LMF", "RMF", "DMF", "CMF", "AMF", "SS", "CF"],

    # =================================================================
    # 🛡️ 3 HẬU VỆ (BIẾN THỂ PHỨC TẠP)
    # =================================================================
    "3-2-4-1 (2 AMF)":              ["GK", "CB", "CB", "CB", "DMF", "DMF", "LMF", "RMF", "AMF", "AMF", "CF"], # Man City 2023
    "3-2-3-2 (WM Modern)":          ["GK", "CB", "CB", "CB", "DMF", "DMF", "LMF", "RMF", "AMF", "CF", "CF"],
    "3-1-4-2 (Conte)":              ["GK", "CB", "CB", "CB", "DMF", "LMF", "RMF", "AMF", "AMF", "CF", "CF"],
    ...
    # =================================================================
    # 🛡️ 5 HẬU VỆ (PARKING BUS & COUNTER)
    # =================================================================
    "5-3-2 (Block)":                ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "DMF", "CMF", "CF", "CF"],
    "5-3-2 (Counter)":              ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "AMF", "AMF", "CF", "CF"],
    "5-3-2 (2 SS)":                 ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "CMF", "CMF", "SS", "SS"],
    "5-3-2 (Flat Mid)":             ["GK", "LB", "CB", "CB", "CB", "RB", "CMF", "CMF", "CMF", "CF", "CF"],

    "5-2-1-2 (2 DMF)":              ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "CF", "CF"],
    "5-2-1-2 (1 DMF)":              ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "CMF", "AMF", "CF", "CF"],
    "5-2-1-2 (3 CMF)":              ["GK", "LB", "CB", "CB", "CB", "RB", "CMF", "CMF", "AMF", "CF", "CF"],

    "5-4-1 (Flat)":                 ["GK", "LB", "CB", "CB", "CB", "RB", "LMF", "RMF", "DMF", "CMF", "CF"],
    "5-4-1 (Diamond)":              ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "LMF", "RMF", "AMF", "CF"],
    "5-4-1 (2 AMF)":                ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "AMF", "CF"],
    
    "5-1-3-1":                      ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "AMF", "AMF", "AMF", "CF"],

    "5-2-3 (Long Ball)":            ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "DMF", "LWF", "RWF", "CF"],
    "5-2-3 (3 SS)":                 ["GK", "LB", "CB", "CB", "CB", "RB", "DMF", "CMF", "SS", "SS", "SS"],
    "5-2-3 (Attack)":               ["GK", "LB", "CB", "CB", "CB", "RB", "CMF", "AMF", "LWF", "RWF", "CF"],

    # =================================================================
    # ⚠️ SƠ ĐỒ ĐẶC BIỆT / KHÔNG TIỀN ĐẠO (ZERO STRIKER)
    # =================================================================
    "4-6-0 (False 9)":              ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "CMF", "AMF", "SS", "SS"],
    "4-6-0 (Magic Box)":            ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "AMF", "AMF", "SS", "SS"],
    "3-6-1 (Possession)":           ["GK", "CB", "CB", "CB", "DMF", "DMF", "LMF", "RMF", "AMF", "AMF", "SS"],
    "3-7-0 (All Midfield)":         ["GK", "CB", "CB", "CB", "DMF", "DMF", "LMF", "RMF", "CMF", "AMF", "SS"],
    "4-2-4-0 (4 SS)":               ["GK", "LB", "CB", "CB", "RB", "DMF", "DMF", "SS", "SS", "SS", "SS"],
    "2-3-5 (Inverted Pyramid)":     ["GK", "LB", "CB", "CB", "RB", "DMF", "CMF", "AMF", "LWF", "RWF", "SS"], # Fake 2-3-5 using 4-back logic
}
